/*
 ...
 Description : Sets up the general interrupt controller (GIC-600) for timer in
               non secure group 1
...
.equ GICDbase,		            0x30000000 //Morello GICDbase base address
...
gicInitN:
  ...
  // Power on the Redistributor(FOLLOWED PROCEDURE from gic-x00.c in trusted firmware from c code)
...
1:   // We now have to wait for sleep to read 0
  ...
  MOV      w2, #0x0
  MOV      w2, #(1 << 30) //EL1 physical timer(ID 30) non secure group 1
  STR      w2, [x1]
  // STR      wzr, [x1]          // Mark all IDs 0..31 as Secure
  ...
  MOV      w2, #0x0
  MOV      w2, #(1 << 30) //EL1 physical timer(ID 30)- Reserved treated as non secure group 1
  STR      w2, [x1]
  //STR      wzr, [x1]          // Mark INTIDs 0..31 as Secure Group 0

  // Interrupt Set-Enable Registers
  // added for non-secure interupt source TIMER IS 30
  // Enables forwarding of the corresponding PPI to the CPU interfaces.
  ADD      x1, x0, #GICR_ISENABLERoffset
  MOV      w2, #(1 << 30)     // Enable INTID 30
  STR      w2, [x1]           // Enable interrupt source

  // Non-secure Access Control Register
  // Configures the level of Non-secure access permitted when the SGI is in Secure Group 0 or Secure Group 1,
  // as defined from GICR_IGROUPR0 and GICR_IGRPMODR0.
  // A field is provided for each SGI. The possible values of each 2-bit field are:
  ...
  // 10 - Non-secure writes are permitted to generate a Secure Group 0 SGI, and
  //     Non-secure writes are permitted to generate a Secure Group 1 SGI.
  // 11 - Reserved
  // NOT SURE IF THIS IS NEEDED, BUT HAVE INCLUDED FOR NOW
  ADD      x1, x0, #GICR_NSACRoffset
  MOV      w2, #(1 << 30)     // Enable INTID 30
  STR      w2, [x1]           // Enable interrupt source
  ISB
  //*********************************************************************
